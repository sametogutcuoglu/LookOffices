...
final class ListOfficesPresenter: ListOfficesPresentationLogic {
    ...
    func presentOffices(response: ListOffices.FetchOffices.Response) {
        ...
        for office in response.Offices {
            let displayOffice = ListOffices.FetchOffices.ViewModel.DisplayedOffice(address: office.address , capacity: office.capacity, id: office.id, image: office.image, location: office.location, name: office.name, rooms: office.rooms, space: office.space)
